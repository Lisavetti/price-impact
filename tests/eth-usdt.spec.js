...
const ethQty   = usdBuckets.map(u => (+binance['ETHUSDT'].qty[u]).toFixed(8)); 

test('scrap ETH/USDT and calculate Price Impact', async ({ page }) => {
  const pair = {};
  const results = {};

  await page.goto('https://app.1inch.io/#/1/simple/swap/1:ETH/1:USDT');
  const input = page.locator('input[automation-id="tui-primitive-textfield__native-input"]');

  for (const amount of ethQty) {
    ...
    if (price) {
      pair[amount] = price;
      console.log(`Курс для ${amount} ETH: ${price}`);
    }
  }

  const baseQty   = ethQty[0];       
  const basePrice = pair[baseQty];

  results["ETH/USDT"] = { prices: pair, impacts: {} };
  ...
  for (const [amount, price] of Object.entries(pair)) {
    ...
    results["ETH/USDT"].impacts[amount] = impact.toFixed(5);
    console.log(`• ${amount} ETH → PI: ${impact.toFixed(5)}`);
  }

  fs.writeFileSync('results_eth-usdt.json', JSON.stringify(results, null, 2));
});

test('scrap USDT/ETH and calculate Price Impact', async ({ page }) => {
  const testAmounts = ['1000', '100000', '500000', '1000000', '2000000', '5000000'];
  const pair = {};
  const results = {};

  await page.goto('https://app.1inch.io/#/1/simple/swap/1:USDT/1:ETH');
  const input = page.locator('input[automation-id="tui-primitive-textfield__native-input"]');
  ...
  for (const amount of testAmounts) {
    ...
    await input.first().fill(amount);
    const rate = page.locator('span[automation-id="tui-accordion__item-title"]');
    await expect(rate).not.toContainText('0 0 ETHFree', { timeout: 10000 });
  ...
  results["USDT/ETH"] = { prices: pair, impacts: {} };
  console.log('\nРезультати Price Impact:\n');

  for (const [amount, price] of Object.entries(pair)) {
    if (amount === "1000") continue;
    const impact = Math.abs((price - basePrice) / basePrice);
    results["USDT/ETH"].impacts[amount] = impact.toFixed(5);
    console.log(`• ${amount} USDT → PI: ${impact.toFixed(5)}`);
  }
  fs.writeFileSync('results_usdt-eth.json', JSON.stringify(results, null, 2));
});
